...
public class LearningService {
    ...
    public QuizSubmissionResponseDTO submitQuizResults(Long userId, List<QuizResultDTO> results) {
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            UserVocabulary uv = userVocabRepo
                    .findByUser_IdAndVocabulary_Id(userId, result.getVocabularyId())
                    .orElse(null);

            if (uv != null) {
                uv.setStatus(isCorrectResult ? LearningStatus.LEARNED : LearningStatus.NOT_LEARNED);
                userVocabRepo.save(uv);
                log.info("  - Updated UV status to {}", uv.getStatus());
            } else {
                log.warn("  - WARNING: UserVocabulary not found for vocabId: {} (userId={})", result.getVocabularyId(), userId);
            }
        }
